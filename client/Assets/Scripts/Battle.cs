...
public class Battle : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (
            characterClientReference == null
            && GameServerConnectionManager.Instance.players.Count != 0
        )
        {
            characterClientReference = Utils
                .GetPlayer(GameServerConnectionManager.Instance.playerId)
                    ...
                    )
                    {
                        foreach (OldActionTracker actionTracker in serverPlayerUpdate.Action)
                        {
                            if (
                                ...
                                && !playerCharacter.GetCurrentAction().Contains(actionTracker)
                            )
                            {
                                playerCharacter.AddToCurrentActions(actionTracker);
                                playerCharacter
                                    .GetComponent<Skill>()
                        ...
                        {
                            playerCharacter.RemoveFromCurrentActions(action);
                        }
    ...
    )
    {
        ...
        Animator modelAnimator = player.CharacterModel.GetComponent<Animator>();
        ...
        player.RotateCharacterOrientation();
